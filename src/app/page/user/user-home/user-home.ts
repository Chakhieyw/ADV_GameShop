...
export class UserHome implements OnInit {
  ...
  performSearch() {
    ...
    if (this.searchText.trim()) {
      ...
      results = results.filter(
        (g) => g.name && g.name.toLowerCase().includes(text)
      );
    }
  ...
  async loadOwnedGames() {
    ...
    try {
      const querySnapshot = await getDocs(
        collection(this.firestore, 'mygames')
      );
      const list: any[] = [];
  ...
  async loadGames() {
    try {
      ...
      const top = list
        .filter((g) => (g.sold || 0) > 0) // เอาเฉพาะเกมที่มีการขายจริง
  ...
  async checkout() {
    ...
    if (this.selectedDiscount()) {
      const usedBefore = await this.hasUserUsedCoupon(
        userData.uid,
    ...
    try {
      ...
      await this.gameService.updateUserMoney(
        userData.uid,
